...
12. **CMS TBA state approved.** Courses use `cohort.status` with `scheduled` or `tba`. When a course is `tba`, the public site must show a clear "Date to be announced" state, hide the countdown, and render no venue anywhere for that course. Arabic-context fallback fields should be Arabic; mono visual labels may stay English.
